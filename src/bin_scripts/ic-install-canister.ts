...
import {
  ...
  ICDevKitConfigurationCanisterSectionItem,
} from "../src/ICDevKitConfiguration";
...
import {
  addCanisterToDfxJson,
  AddCanisterToDfxJsonStatus,
} from "../src/dfxJson";

const npm = {
  installNpmPackage: (packageName: string, isSave?: boolean) => {
    logger.info(`Installing npm package ${packageName}`);
    const result = exec(`npm install ${packageName} ${isSave ? "--save" : ""}`);
    if (result.code !== 0) {
...
async function installCanister(
  name: string,
  config: ICDevKitConfigurationCanisterSectionItem
): Promise<void> {
  ...
  if ("command" in config.install) {
    // install by command
    logger.debug(
      `Installing canister: ${name} by command: ${config.install.command}`
    );
    // install by command
    const result = exec(config.install.command);
    if (result.code !== 0) {
...
export const execute_task_install_canister = async (
  options: IInstallCanisterInput
) => {
  ...
  for (const canister_name of canister_to_be_install) {
    await installCanister(
      canister_name,
      icDevKitConfiguration.install_canisters[canister_name]
    );
  }
};
