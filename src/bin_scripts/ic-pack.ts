...
const pack_npm_client = (input: PackNpmClientInput) => {
  ...
  fs.cpSync(`${npm_client_template_dir}/`, `${input.target_dir_path}/`, {
    recursive: true,
  });
  ...
  fs.writeFileSync(
    package_json,
    JSON.stringify(package_json_content_obj, null, 2)
  );

  const generate_bind = (target: string) => {
    const result = exec(
      `npx ic-didc bind ${input.did_file_path} --target ${target}`,
      { silent: true }
    );
    if (result.code !== 0) {
      logger.error(
        `npm client generate bind error for ${input.did_file_path}: ${result.stdout}`
      );
      return "";
    }
  ...
  // run tsc to build ts
  const tsc_result = exec(`cd ${input.target_dir_path} && npm run build`, {
    silent: false,
  });
  if (tsc_result.code !== 0) {
    logger.error(
      `npm client tsc error for ${input.did_file_path}: ${tsc_result.stderr}`
    );
    return;
  }
};
...
const pack_npm_server = (input: PackNpmServerInput) => {
  ...
  fs.cpSync(`${npm_server_template_dir}/`, `${input.target_dir_path}/`, {
    recursive: true,
  });
...
const build_all = async (buildContext: BuildContext) => {
  ...
  for (const name of Object.keys(buildContext.canisters)) {
  ...
  const canister_envs = buildContext.canister_envs;
  ...
  for (const canisterEnv of canister_envs) {
    ...
    const isProductionEnv =
      canisterEnv == buildContext.icPackInput.productionCanisterEnv;
    logger.debug(
      `build canister_env ${canisterEnv}, isProductionEnv: ${isProductionEnv}`
    );

    for (const [name, canister_json] of Object.entries(
      buildContext.canisters
    )) {
    ...
    for (const file of files) {
      if (file.endsWith(".wasm")) {
        ...
        if (stat.size > 2 * 1024 * 1024) {
          logger.warn(
            `WASM file size of ${file} is ${stat.size} bytes, must be < 2MB`
          );
        }
      }
    }
  ...
  for (const canister_env of buildContext.canister_envs) {
    ...
    for (const category of categoryDirs) {
      ...
      for (const packageName of packageDirs) {
        ...
        const result = exec(`cd ${packageDir} && ${publishComamnd}`, {
          silent: true,
        });
        if (result.code != 0) {
          logger.error(
            `Failed to publish ${packageDir}. Error: ${result.stderr}`
          );
        } else {
...
export const execute_task_pack = async (input: ICPackInput) => {
  ...
  if (input.canisterEnv) {
    if (!all_envs.has(input.canisterEnv)) {
      logger.error(
        `Canister env ${input.canisterEnv} not found in ${DEFAULT_DFX_PACKAGE_JSON_FILENAME}`
      );
      return;
    }
