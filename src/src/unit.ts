...
const parseToCommon = (
  ...
  if (precision) bn = new BigNumber(new BigNumber(bn.toFixed(precision)).toPrecision());
  return bn;
};

const parseToOrigin = (commonQty: bigint | string | BigNumber | number, tokenDecimals: number): bigint => {
  // commonQty * 10^tokenDecimals
  if (typeof commonQty === 'string') {
    // replace all _ with ''
    commonQty = commonQty.replace(/_/g, '');
  }
  const bigNumber = new BigNumber(commonQty.toString()).times(new BigNumber(10).pow(tokenDecimals));
  return BigInt(bigNumber.toFixed());
};

const toPrecision = (originTokenQty: bigint | string | BigNumber, precision?: number): BigNumber => {
  let res: string;
