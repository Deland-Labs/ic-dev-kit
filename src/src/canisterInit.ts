...
export const parseICLedgerInit = (
  input: ICLedgerInitParameter
): ArrayBuffer => {
  ...
  const args = {
    ...
    transaction_window: p.transaction_window
      ? [
          {
            secs: BigInt(p.transaction_window.secs),
            nanos: p.transaction_window.nanos,
          },
        ]
      : [],
    max_message_size_bytes: p.max_message_size_bytes
      ? [BigInt(p.max_message_size_bytes)]
      : [],
    archive_options: p.archive_options
      ? [
          {
            num_blocks_to_archive: BigInt(
              p.archive_options.num_blocks_to_archive
            ),
            trigger_threshold: BigInt(p.archive_options.trigger_threshold),
            max_message_size_bytes: p.archive_options.max_message_size_bytes
              ? [BigInt(p.archive_options.max_message_size_bytes)]
              : [],
            node_max_memory_size_bytes: p.archive_options
              .node_max_memory_size_bytes
              ? [BigInt(p.archive_options.node_max_memory_size_bytes)]
              : [],
            controller_id: Principal.fromText(p.archive_options.controller_id),
          },
        ]
      : [],
    initial_values: p.initial_values
      ? p.initial_values.map((x) => [x.account, { e8s: BigInt(x.icpts) }])
      : [],
  };
      ...
      ? [
          {
            ...
            max_message_size_bytes: input.archive_option.max_message_size_bytes
              ? [input.archive_option.max_message_size_bytes]
              : [],
            cycles_for_archive_creation: input.archive_option
              .cycles_for_archive_creation
              ? [BigInt(input.archive_option.cycles_for_archive_creation)]
              : [],
            node_max_memory_size_bytes: input.archive_option
              .node_max_memory_size_bytes
              ? [input.archive_option.node_max_memory_size_bytes]
              : [],
          },
        ]
      : [],
