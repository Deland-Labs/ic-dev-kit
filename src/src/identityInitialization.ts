...
const new_identity = (name: string) => {
  const result = exec(`dfx identity new --disable-encryption ${name}`, {
    silent: false,
  });
  if (result.code !== 0) {
...
export class IdentityInitialization {
  ...
  import_identity = (name: string) => {
    ...
    const target_pem_path = get_pem_path(name);
    {
      ...
      const result = exec(`chmod 777 ${target_pem_path}`, { silent: false });
      if (result.code !== 0) {
        logger.error(result.stderr);
        throw new Error(`Failed to chmod 777 ${target_pem_path}`);
      }
    }
    const source_pem_path = `${this._configuration.pem_source_dir}/${name}.pem`;
    if (!fs.existsSync(source_pem_path)) {
      logger.warn(
        `there is no identity.pem in ${source_pem_path}, it could be unstable for dev env`
      );
    } else {
  ...
  initAllIdentities() {
    const identityNames = this.getIdentityPemNames();
    if (identityNames.length == 0) {
      const defaultIdentity = icDevKitConfiguration.identity.default_identity;
      logger.info(
        `There is no identity need to be import, use default identity: ${defaultIdentity}`
      );
    }
