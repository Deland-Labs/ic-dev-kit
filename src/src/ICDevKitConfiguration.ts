...
export const LoadICDevKitConfiguration = (filepath?: string): ICDevKitConfiguration => {
  let config: {} = {};
  let default_config = {
    identity: {
  ...
  try {
    config = JSON.parse(fs.readFileSync(filepath ?? IC_DEV_KIT_CONFIGURATION_FILE_NAME).toString());
