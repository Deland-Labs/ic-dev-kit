...
export const get_wasm_path = (
  name: string,
  canister: DfxJsonCanister
): string => {
  if (canister?.wasm) {
...
export const addCanisterToDfxJson = (
  input: addCanisterToDfxJsonInput,
  dfxJsonPath?: string
): AddCanisterToDfxJsonStatus => {
  const dfxJsonFilepath = dfxJsonPath ?? "./dfx.json";
  const json = fs.readFileSync(dfxJsonFilepath, "utf8");
