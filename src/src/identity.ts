...
export const getDfxPrincipal = (): string => {
  const result = exec(`dfx identity get-principal`, { silent: true });
  return result.trim();
};
...
export class IdentityFactory {
  ...
  ensureIdentityLoaded() {
    if (this._identities.size == 0) {
      const identityNames = identityInitialization.getIdentityPemNames();
  ...
  getPrincipal = (name?: string): Principal | undefined => {
    ...
    const identityInfo = this.getIdentity(
      name || this.getDefaultIdentityName()
    );
    if (identityInfo) {
