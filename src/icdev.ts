...
import {
  DEFAULT_BUILD_ENV_NAME,
  DEFAULT_DECLARATIONS_OUT_DIR,
...
program
  ...
  .option(
    "-o, --declarations-out-dir <declarationsOutDir>",
    "output directory for generated ts bindings",
    DEFAULT_DECLARATIONS_OUT_DIR
  )
  .action((options, command) => {
    const input = {
...
program
  ...
  .option(
    "-p, --package-version <package-version>",
    "version of package",
    DEFAULT_PACKAGE_VERSION
  )
  .option(
    "-s, --package-scope <package-scope>",
    "scope of package",
    DEFAULT_PACKAGE_SCOPE
  )
  .option(
    "-n, --canister-env-name <canister-env-name>",
    "enviroment variable name to be set before run `dfx build`",
    DEFAULT_BUILD_ENV_NAME
  )
  .option(
    "-e, --canister-env <canister-env>",
    "canister env to pack, if not specified, pack all canisters",
    ""
  )
  .option(
    "-r, --production-canister-env <production-canister-env>",
    "canister env to pack in production mode.",
    DEFAULT_PRODUCTION_ENV
  )
...
program
  ...
  .action((principal) => {
    execute_task_get_account_id(principal);
  });
